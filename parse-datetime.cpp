...
# define SIGNED_TYPE_OR_EXPR(t) 1
...
#define INT_STRLEN_BOUND(t)                                     \
  (INT_BITS_STRLEN_BOUND (TYPE_WIDTH (t) - SIGNED_TYPE_OR_EXPR (t)) \
   + SIGNED_TYPE_OR_EXPR (t))
...
{
    return _addcarry_u32(0, a, b, (unsigned int*)res) != 0;
}

bool add_overflow(long int a, long int b, long int* res) { return _addcarry_u32(0, a, b, (unsigned int*)res) != 0; }
bool add_overflow(unsigned int a, unsigned int b, unsigned int* res) { return _addcarry_u32(0, a, b, res) != 0; }
bool add_overflow(unsigned long int a, unsigned long int b, unsigned long int* res) { return _addcarry_u32(0, a, b, (unsigned int*)res) != 0; }
...
bool sub_overflow(int a, int b, int* res) { return _subborrow_u32(0, a, b, (unsigned int*)res) != 0; }
bool sub_overflow(long int a, long int b, long int* res) { return _subborrow_u32(0, a, b, (unsigned int*)res) != 0; }
bool sub_overflow(unsigned int a, unsigned int b, unsigned int* res) { return _subborrow_u32(0, a, b, res) != 0; }
bool sub_overflow(unsigned long int a, unsigned long int b, unsigned long int* res) { return _subborrow_u32(0, a, b, (unsigned int*)res) != 0; }
...
bool sub_overflow(long long a, long long b, long long* res)
{
    ...
    ret = _subborrow_u32(0, a, b, &tmp) != 0;
    if (res != nullptr)
    {
        *res = tmp;
    }
...
{
    long long res64 = __emul(a, b);
    if (res != nullptr)
    {
        *res = (int)(res64 & 0xffffffff);
    }
    return ((int)(res64 >> 32)) & 0xffffffff;
}

bool mul_overflow(unsigned int a, unsigned int b, unsigned int* res)
{
    unsigned long long res64 = __emulu(a, b);
    if (res != nullptr)
    {
        *res = (int)(res64 & 0xffffffff);
    }
...
bool mul_overflow(long int a, long int b, long int* res)
{
    long long res64 = __emul(a, b);
    if (res != nullptr)
    {
...
bool mul_overflow(unsigned long int a, unsigned long int b, unsigned long int* res)
{
    unsigned long long res64 = __emulu(a, b);
    if (res != nullptr)
    {
...
#if defined(_M_AMD64)
bool add_overflow(long long int a, long long int b, long long int* res) { return _addcarry_u64(0, a, b, (unsigned long long*)res) != 0; }
bool add_overflow(unsigned long long int a, unsigned long long int b, unsigned long long int* res) { return _addcarry_u64(0, a, b, res) != 0; }

bool sub_overflow(long long int a, long long int b, long long int* res) { return _subborrow_u64(0, a, b, (unsigned long long*)res) != 0; }
bool sub_overflow(unsigned long long int a, unsigned long long int b, unsigned long long int* res) { return _subborrow_u64(0, a, b, res) != 0; }

bool mul_overflow(long long int a, long long int b, long long int* res)
{
    __int64 hi = 0;
    if (res != nullptr)
    {
...
bool mul_overflow(unsigned long long int a, unsigned long long int b, unsigned long long int* res)
{
    __int64 hi = 0;
    if (res != nullptr)
    {
  ...
  if (pc->year_seen != pc->debug_year_seen)
    {
      if (space)
        fputc (' ', stderr);
      fprintf (stderr, ("year: %04" PRIdMAX), pc->year.value);
  ...
  if (pc->days_seen && !pc->debug_days_seen)
    {
      if (space)
        fputc (' ', stderr);
      char tmp[DBGBUFSIZE];
      fprintf (stderr, ("%s (day ordinal=%" PRIdMAX " number=%d)"),
  ...
  if (pc->timespec_seen)
    {
      ...
      if (space)
        fputc (' ', stderr);
      fprintf (stderr, ("number of seconds: %" PRIdMAX), sec);
    }
...
#   define YY(Msgid) dgettext ("bison-runtime", Msgid)
#  endif
# endif
# ifndef YY_
#  define YY(Msgid) Msgid
# endif
#endif
...
{
       0,   598,   598,   599,   603,   611,   613,   617,   622,   627,
     632,   637,   642,   647,   652,   656,   660,   667,   671,   675,
     680,   685,   690,   694,   699,   704,   711,   713,   717,   742,
     744,   754,   756,   758,   763,   768,   771,   773,   778,   783,
     788,   794,   803,   808,   841,   849,   857,   862,   868,   873,
     879,   883,   893,   895,   897,   902,   904,   906,   908,   910,
     912,   914,   917,   920,   922,   924,   926,   928,   930,   932,
     934,   936,   938,   940,   942,   944,   948,   950,   952,   955,
     957,   959,   964,   968,   968,   971,   972,   978,   979,   985,
     990,  1001,  1002
};
#endif
...
#define YYBACKUP(Token, Value)                                    \
  do                                                              \
    if (yychar == YYEMPTY)                                        \
      ...
      {                                                           \
        yyerror (pc, YY("syntax error: cannot back up")); \
        YYERROR;                                                  \
      }                                                           \
  while (0)
...
# if defined yyoverflow
      {
        ...
        yyoverflow (YY("memory exhausted"),
...
yyreduce:
  ...
  case 15: /* item: number  */
      {
        debug_print_current_time (("number"), pc);
      }
    break;
  ...
  case 43: /* date: tUNUMBER '/' tUNUMBER '/' tUNUMBER  */
      {
        ...
        if (4 <= (yyvsp[-4].textintval).digits)
            ...
            if (debugging (pc))
              {
                ...
                dbg_printf (("warning: value %" PRIdMAX " has %" PRIdMAX " digits. "
                              "Assuming YYYY/MM/DD\n"),
          ...
          {
            if (debugging (pc))
              dbg_printf (("warning: value %" PRIdMAX " has less than 4 digits. "
...
`-----------------------------------------------------------*/
yyexhaustedlab:
  yyerror (pc, YY("memory exhausted"));
  yyresult = 2;
  goto yyreturnlab;
...
to_tm_year (textint textyear, bool debug, int *tm_year)
{
  ...
  if (0 <= year && textyear.digits == 2)
    {
      ...
      if (debug)
        dbg_printf (("warning: adjusting year value %" PRIdMAX
                      " to %" PRIdMAX "\n"),
                    textyear.value, year);
    }
  ...
  if (year < 0
    ...
    {
      if (debug)
        dbg_printf (("error: out-of-range year %" PRIdMAX "\n"), year);
      return false;
    }
      ...
      if (isalpha (c))
        {
          ...
          *p = '\0';
          ...
          if (! tp)
            {
              if (debugging (pc))
                dbg_printf (("error: unknown word '%s'\n"), buff);
              return '?';
            }
                     ...
                     parser_control const *pc, bool time_zone_seen)
{
  ...
  dbg_printf (("error: invalid date/time value:\n"));
  dbg_printf (("    user provided time: '%s'\n"),
              debug_strfdatetime (tm0, pc, tmp, sizeof tmp));
  dbg_printf (("       normalized time: '%s'\n"),
  ...
  dbg_printf (("     possible reasons:\n"));
  if (dst_shift)
    dbg_printf (("       nonexistent due to daylight-saving time;\n"));
  if (!eq_mday && !eq_month)
    dbg_printf (("       invalid day/month combination;\n"));
  dbg_printf (("       numeric values overflow;\n"));
  dbg_printf ("       %s\n", (time_zone_seen ? ("incorrect timezone")
                              : ("missing timezone")));
}
                     ...
                     timezone_t tzdefault, char const *tzstring)
{
...
#endif
  struct tm tmp;
  if (localtime_s(&tmp, &now->tv_sec) != 0)
      goto fail;
  ...
  if (ckd_add (&pc.year.value, tmp.tm_year, TM_YEAR_BASE))
    {
      if (debugging (&pc))
        dbg_printf (("error: initial year out of range\n"));
      goto fail;
    }
  pc.year.digits = 0;
  ...
  for (int quarter = 1; quarter <= 3; quarter++)
    {
      ...
      if (ckd_add (&probe, Start, quarter * (90 * 24 * 60 * 60)))
        break;
      struct tm probe_tm;
      if (localtime_s(&probe_tm, &probe) == 0
  ...
  if (debugging (&pc))
    {
      dbg_printf (("input timezone: "));

      if (pc.timespec_seen)
        fprintf (stderr, ("'@timespec' - always UTC"));
      else if (pc.zones_seen)
        fprintf (stderr, ("parsed date/time string"));
      else if (tzstring)
        {
          if (tz != tzdefault)
            fprintf (stderr, ("TZ=\"%s\" in date string"), tzstring);
          else if (STREQ (tzstring, "UTC0"))
            {
              /* Special case: 'date -u' sets TZ="UTC0".  */
              fprintf (stderr, ("TZ=\"UTC0\" environment value or -u"));
            }
          else
            fprintf (stderr, ("TZ=\"%s\" environment value"), tzstring);
        }
      else
        fprintf (stderr, ("system default"));
    ...
    *result = pc.seconds;
  else
    {
      ...
      if (! to_tm_year (pc.year, debugging (&pc), &tm.tm_year)
          || ckd_sub (&tm.tm_mon, pc.month, 1)
          || ckd_add (&tm.tm_mday, pc.day, 0))
        {
          if (debugging (&pc))
            dbg_printf (("error: year, month, or day overflow\n"));
          goto fail;
        }
          ...
          if (tm.tm_hour < 0)
            {
              ...
              if (debugging (&pc))
                dbg_printf (("error: invalid hour %" PRIdMAX "%s\n"),
                            pc.hour, mrd);
              goto fail;
            }
          ...
          if (time_zone_seen)
            {
              ...
              if (!tz2)
                {
                  if (debugging (&pc))
                    dbg_printf (("error: tzalloc (\"%s\") failed\n"), tz2buf);
                  goto fail;
                }
      ...
      if (pc.days_seen && ! pc.dates_seen)
        {
          ...
          if (debugging (&pc))
            dbg_printf (("new start date: '%s' is '%s'\n"),
                        str_days (&pc, dbg_ord, sizeof dbg_ord),
      ...
      if (debugging (&pc))
        {
          if (!pc.dates_seen && !pc.days_seen)
            dbg_printf (("using current date as starting value: '%s'\n"),
                        debug_strfdate (&tm, dbg_tm, sizeof dbg_tm));

          if (pc.days_seen && pc.dates_seen)
            dbg_printf (("warning: day (%s) ignored when explicit dates "
                          "are given\n"),
                        str_days (&pc, dbg_ord, sizeof dbg_ord));

          dbg_printf (("starting date/time: '%s'\n"),
                      debug_strfdatetime (&tm, &pc, dbg_tm, sizeof dbg_tm));
        }
      ...
      if (pc.rel.year | pc.rel.month | pc.rel.day)
        {
          if (debugging (&pc))
            {
              if ((pc.rel.year != 0 || pc.rel.month != 0) && tm.tm_mday != 15)
                dbg_printf (("warning: when adding relative months/years, "
                              "it is recommended to specify the 15th of the "
                              "months\n"));

              if (pc.rel.day != 0 && tm.tm_hour != 12)
                dbg_printf (("warning: when adding relative days, "
          ...
          if (tm.tm_wday < 0)
            {
              if (debugging (&pc))
                dbg_printf (("error: adding relative date resulted "
          ...
          if (debugging (&pc))
            {
              ...
              */
              if (tm0.tm_isdst != -1 && tm.tm_isdst != tm0.tm_isdst)
                dbg_printf (("warning: daylight saving time changed after "
                              "date adjustment\n"));
              ...
              */
              if (pc.rel.day == 0
                ...
                {
                  dbg_printf (("warning: month/year adjustment resulted in "
                                "shifted dates:\n"));
                  char tm_year_buf[TM_YEAR_BUFSIZE];
                  dbg_printf (("     adjusted Y M D: %s %02d %02d\n"),
                              tm_year_str (year, tm_year_buf), month + 1, day);
                  dbg_printf (("   normalized Y M D: %s %02d %02d\n"),
                              tm_year_str (tm.tm_year, tm_year_buf),
                              tm.tm_mon + 1, tm.tm_mday);
                }
            }
      ...
      if (pc.zones_seen)
        {
          ...
          if (overflow)
            {
              if (debugging (&pc))
                dbg_printf (("error: timezone %d caused time_t overflow\n"),
                            pc.time_zone);
              goto fail;
            }
          Start = t1;
      ...
      if (debugging (&pc))
        {
          ...
          dbg_printf (("'%s' = %" PRIdMAX " epoch-seconds\n"),
                      debug_strfdatetime (&tm, &pc, dbg_tm, sizeof dbg_tm),
                      Starti);
        }
         ...
         again the time zone will be lost.  */
      {
        ...
        if (ckd_mul (&d1, pc.rel.hour, 60 * 60)
          ...
          {
            if (debugging (&pc))
              dbg_printf (("error: adding relative time caused an "
                            "overflow\n"));
            goto fail;
          }
        ...
        if (debugging (&pc)
          ...
          {
            dbg_printf (("after time adjustment (%+" PRIdMAX " hours, "
                          "%+" PRIdMAX " minutes, "
                          "%+" PRIdMAX " seconds, %+d ns),\n"),
                        pc.rel.hour, pc.rel.minutes, pc.rel.seconds,
                        pc.rel.ns);
            intmax_t t4i = t4;
            dbg_printf (("    new time = %" PRIdMAX " epoch-seconds\n"), t4i);
            ...
            struct tm lmt;
            if (tm.tm_isdst != -1 && localtime_s(&lmt, &result->tv_sec) == 0
  ...
  if (debugging (&pc))
    {
      ...
      if (! tzstring)
        dbg_printf (("timezone: system default\n"));
      else if (STREQ (tzstring, "UTC0"))
        dbg_printf (("timezone: Universal Time\n"));
      else
        dbg_printf (("timezone: TZ=\"%s\" environment value\n"), tzstring);

      intmax_t sec = result->tv_sec;
      int nsec = result->tv_nsec;
      dbg_printf (("final: %" PRIdMAX ".%09d (epoch-seconds)\n"),
                  sec, nsec);

      struct tm gmt, lmt;
      bool got_utc = gmtime_s(&gmt, &result->tv_sec) == 0;
      if (got_utc)
        dbg_printf (("final: %s (UTC)\n"),
                    debug_strfdatetime (&gmt, NULL,
                                        dbg_tm, sizeof dbg_tm));
        if (localtime_s(&lmt, &result->tv_sec) == 0)
        {
#ifdef HAVE_STRUCT_TM_TM_GMTOFF
                ...
                struct timespec const *now)
{
  ...
  return  parse_datetime_body(result, p, now, 0, nullptr, tzstring);
}
